...
class Viewer
{
    ...
    openFigma()
    {
        const figmaPage = story.groups.filter(g => g.id === this.currentPage.groupID)[0]
        let url = `https://www.figma.com/file/${story.fileKey}/${encodeURIComponent(story.docName)}?page-id=${encodeURIComponent(figmaPage.id)}&node-id=${encodeURIComponent(this.currentPage.id)}`
        window.open(url);
    }
    showChild(child)
    {
    ...
    refresh_adjust_content_layer(page)
    {
        ...
        var prevPageWasModal = this.prevPage && this.prevPage.type === "modal"
        if (prevPageWasModal)
        {
